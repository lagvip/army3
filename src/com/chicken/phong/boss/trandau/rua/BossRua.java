 ...
 * Boss Rùa dùng hình BigBoss native của client, đuổi người gần nhất và gây sát thương khi chạm.
 */
public final class BossRua extends ChickenQuanLyChien {
    ...
    public BossRua(SanhChoBoss sanh) {
        super(null, layNguoiChoiTheoGhe(sanh), (byte) CauHinhBossRua.MAP_ID);
        this.sanh = sanh;
    ...
    private void taoBoss() {
        for (CauHinhBossRua.CauHinh cauHinh : CauHinhBossRua.layTatCa()) {
            ...
            int tanCong = cauHinh.laBossBanSung()
                    ? CauHinhBossRua.layTanCongTheoSung(cauHinh.getVuKhi())
                    : 0;
    ...
    private void thucHienLuotBoss(int slot, long phien) {
        synchronized (this) {
            ...
            ChickenChienBinh mucTieu = BossCamTu.timNguoiSongGanNhat(
                    boss, this.chienBinhs);
            int slotMucTieu = mucTieu == null ? -1 : mucTieu.chiSo & 0xFF;
            int huongX = mucTieu == null ? 0 : BossCamTu.layHuongX(boss, mucTieu);
            this.thucHienRua(
                    boss, slot, phien,
    ...
    private void thucHienRua(
        ...
        synchronized (this) {
            ...
            if (mucTieu == null) {
                mucTieu = BossCamTu.timNguoiSongGanNhat(boss, this.chienBinhs);
                slotMucTieu = mucTieu == null ? -1 : mucTieu.chiSo & 0xFF;
                huongXKhoa = mucTieu == null ? 0 : BossCamTu.layHuongX(boss, mucTieu);
            }
            if (mucTieu == null) {
                this.sangLuotSauBoss(slot, phien, CauHinhBossRua.NAP_DAN_SAU_HANH_DONG);
                return;
            }
            if (BossCamTu.daChamNguoiChoi(boss, mucTieu)) {
                this.ruaTanCong(boss, mucTieu, slot, phien);
                return;
            }
            ...
            boss.x = kepShort(buoc[0], ChickenKichThuocNhanVat.BOSS_NUA_RONG,
                    this.banDo.getWidth() - 1 - ChickenKichThuocNhanVat.BOSS_NUA_RONG);
            boss.y = kepShort(buoc[1], 0, this.banDo.getHeight() + 32);
            try {
                this.phatDiChuyenBoss(boss);
            } catch (IOException ignored) {
            }
            if (BossCamTu.daRoiKhoiMap(boss, this.banDo)) {
                try {
                    boss.hp = 0;
                    boss.chet = true;
