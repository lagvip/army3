...
public final class BossHaiToaThap extends ChickenQuanLyChien {
    ...
    /** Bộ kỹ năng AVG riêng dùng đúng mảng 18 chiến binh của trận boss. */
    ...
    public BossHaiToaThap(SanhChoBoss sanh) {
        super(null, layNguoiChoiTheoGhe(sanh), (byte) CauHinhBossHaiToaThap.MAP_ID);
        this.sanh = sanh;
    ...
    public synchronized void batDau() throws IOException {
        ...
        this.daBatDau = true;
        for (CauHinhBossHaiToaThap.CauHinh cauHinh : CauHinhBossHaiToaThap.layTatCa()) {
    ...
    private void taoBoss() {
        for (CauHinhBossHaiToaThap.CauHinh cauHinh : CauHinhBossHaiToaThap.layTatCa()) {
            ...
            int tanCong = cauHinh.laBossBanSung()
                    ? CauHinhBossHaiToaThap.layTanCongTheoSung(cauHinh.getVuKhi())
                    : 0;
    ...
    private int layNapDanKhiHetThoiGian(int slot, ChickenChienBinh chienBinh) {
        ...
        return chienBinh == null
                ? 100
                : CauHinhBossHaiToaThap.layNapDanTheoSung(chienBinh.maVuKhi);
    }
    ...
    private void thucHienBossBanSung(ChickenChienBinh boss, int slot, long phien) {
        ...
        if (mucTieu == null) {
            this.sangLuotSauBoss(slot, phien,
                    CauHinhBossHaiToaThap.layNapDanTheoSung(boss.maVuKhi));
            return;
        }
    ...
    private void noCamTu(
        ...
        try {
            ...
            ChickenKetQuaDan hieuUngNo = new ChickenKetQuaDan(
                    ...
                    xs, ys, mucTieu, CauHinhBossHaiToaThap.SAT_THUONG_CAM_TU);
            this.phatBan(camTu, hieuUngNo, (byte) 1);
            this.banDo.phaDiaHinh(camTu.x, camTu.y, (byte) 15);
            this.gaySatThuong(mucTieu, CauHinhBossHaiToaThap.SAT_THUONG_CAM_TU);
            camTu.hp = 0;
            camTu.chet = true;
            this.phatCapNhatMau(camTu);
    ...
    private VaChamBoss timBossTrung(short[] xs, short[] ys) {
        ...
        for (int i = 1; i < soDiem; i++) {
            ...
            for (int buoc = 1; buoc <= soBuoc; buoc++) {
                ...
                for (int slot = CauHinhBossHaiToaThap.SLOT_BOSS_DAU;
                        slot <= CauHinhBossHaiToaThap.SLOT_BOSS_CUOI; slot++) {
                    ChickenChienBinh boss = this.chienBinhs[slot];
    ...
    private ChickenQuanLyCongThucSung.KiemTraBanDo kiemTraBanDo() {
        return new ChickenQuanLyCongThucSung.KiemTraBanDo() {
            @Override
            public int getWidth() { return BossHaiToaThap.this.banDo.getWidth(); }
            @Override
            public int getHeight() { return BossHaiToaThap.this.banDo.getHeight(); }
            @Override
            public boolean coVaCham(short x, short y) {
                return BossHaiToaThap.this.banDo.coVaCham(x, y);
            }
        };
    }
